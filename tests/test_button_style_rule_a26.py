...
class TestButtonStyleRule(unittest.TestCase):
    ...
    def test_rule_documented(self):
        # GUI-konvensjoner ble flyttet fra INTERFACE.md til DEVELOPMENT.md.
        text = (ROOT / "docs" / "DEVELOPMENT.md").read_text(encoding="utf-8")
        self.assertIn("Primær (blå)", text)
        self.assertIn("Sekundær (mørk)", text)
        self.assertIn("Stopp/fare", text)
