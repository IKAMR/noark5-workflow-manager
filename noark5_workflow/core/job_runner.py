...
class JobRunner:
    ...
    def __init__(self, registry, executor, settings: dict, *, source_factory=Noark5Extraction.detect) -> None:
        self.registry=registry; self.executor=executor; self.settings=settings; self.source_factory=source_factory

    def _configure_operation_for_job(self, job: Job, operation_id: str):
        operation=self.registry.get(operation_id); params=job.get_operation_params(operation_id)
        configure=getattr(operation, "configure", None)
        if params and callable(configure): configure(params)
        return operation

    def continue_job(self, job: Job, *, progress_cb=None, log_cb=None, cancelled_cb=None, state_cb=None) -> JobRunOutcome:
        if job.status != JobStatus.WAITING: raise JobContinueError(f"Jobben kan ikke fortsettes fra status: {job.status.value}")
        total=len(job.workflow_ids); next_index=max(0,min(int(job.next_operation_index),total))
        if next_index <= 0 or next_index >= total: raise JobContinueError("Jobben har ikke en gyldig neste operasjon å fortsette med")
        previous_operation_id=job.workflow_ids[next_index-1]
        if not job.has_checkpoint(previous_operation_id): raise JobContinueError("Jobben står som ventende, men execution cursor følger ikke et kontrollpunkt")
    ...
    def run(self, job: Job, *, progress_cb=None, log_cb=None, cancelled_cb=None, state_cb=None) -> JobRunOutcome:
        ...
        if start_index >= len(op_ids):
            start_index=0; job.next_operation_index=0; job.progress=0.0
        resuming=start_index>0; job.status=JobStatus.RUNNING
        job.message=f"Workflow fortsetter fra operasjon {start_index+1}" if resuming else "Workflow startet"; log(job.message); state_changed()
        output_lock=None
        try:
            if job.output_root:
                output_lock=OutputLock(job.output_root,job.job_id); output_lock.acquire(); log(f"Utdata låst: {job.output_root}")
            ...
            for zero_index in range(start_index,total):
                ...
                if cancelled_cb and cancelled_cb():
                    job.status=JobStatus.SKIPPED; job.message="Avbrutt"; log("AVBRUTT før neste operasjon"); state_changed(); return JobRunOutcome(False,False)
                operation=self._configure_operation_for_job(job,op_id)
                if op_id == "dias_package":
                    params=job.get_operation_params(op_id); configured_output=str(params.get("output_dir","") or "").strip()
                    archive_output=job.archive_root or job.output_root; job_output=str(archive_output) if archive_output is not None else ""
                    if job_output and configured_output != job_output:
                        params["output_dir"]=job_output; job.set_operation_params(op_id,params); operation.configure(params); log(f"DIAS-utdata synkronisert fra jobb: {job_output}")
                log(f"START: {operation.definition.name}"); result=self.executor.execute(operation,ctx); all_ok=all_ok and result.ok; log(result.message)
                for warning in result.warnings: log(f"ADVARSEL: {warning}")
                if result.data: log(json.dumps(result.data,ensure_ascii=False,indent=2))
                log(f"{'OK' if result.ok else 'FEIL'}: {operation.definition.name}"); job.message=result.message
                if not result.ok:
