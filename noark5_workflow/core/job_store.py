...
def _job_to_dict(job: Job) -> dict[str, Any]:
    return {
        ...
        "source_tar": _path_text(job.source_tar),
        "source_unzipped": _path_text(job.source_unzipped),
        "source_extraction": _path_text(job.source_extraction),
        "work_root": _path_text(job.work_root),
        "work_content": _path_text(job.work_content),
        ...
        "status": job.status.value, "progress": float(job.progress), "worker": job.worker,
        "message": job.message, "log_entries": list(job.log_entries),
        "checkpoint_after": list(job.checkpoint_after), "next_operation_index": int(job.next_operation_index),
    }
...
def _job_from_dict(data: dict[str, Any], *, format_version: int) -> Job:
    job_id = str(data.get("job_id", "")).strip(); source_root = str(data.get("source_root", "")).strip()
    if not job_id: raise JobListFormatError("Jobb mangler job_id")
    if not source_root: raise JobListFormatError(f"{job_id} mangler source_root")
    try: status = JobStatus(data.get("status", JobStatus.READY.value))
    except ValueError: status = JobStatus.READY
    message = str(data.get("message", ""))
    if status == JobStatus.RUNNING:
        status = JobStatus.READY; message = "Forrige kjøring var aktiv da jobblisten ble lagret"
    ...
    workflow_ids = [str(v) for v in workflow_ids]
    logs = data.get("log_entries", []); logs = logs if isinstance(logs, list) else []
    if format_version >= 2:
        checkpoints = data.get("checkpoint_after", []); checkpoints = checkpoints if isinstance(checkpoints, list) else []
        checkpoints = [str(v) for v in checkpoints]
        try: next_operation_index = int(data.get("next_operation_index", 0))
        except (TypeError, ValueError): next_operation_index = 0
    else:
        checkpoints = []; next_operation_index = 0
    output_root = _optional_path(data, "output_root")
    return Job(
        job_id=job_id, source_root=Path(source_root), output_root=output_root,
        ...
        name=str(data.get("name", "")), workflow_ids=workflow_ids, operation_params=_json_value(params),
        status=status, progress=progress, worker=str(data.get("worker", "Lokal (denne PC-en)")),
        message=message, log_entries=[str(v) for v in logs][-2000:], checkpoint_after=checkpoints,
...
def save_job_list(path: Path, batch: JobBatch, *, active_job_id: str | None = None, app_version: str = "") -> Path:
    ...
    payload = {"file_type": FILE_TYPE, "format_version": FORMAT_VERSION, "app_version": app_version,
               "created_at": _existing_created_at(path) or now, "modified_at": now,
               "active_job_id": active_job_id, "jobs": [_job_to_dict(job) for job in batch.jobs()]}
...
def load_job_list(path: Path) -> LoadedJobList:
    ...
    if version not in SUPPORTED_FORMAT_VERSIONS:
        raise JobListFormatError(f"Jobblisteformat {version!r} støttes ikke (støttede versjoner: {sorted(SUPPORTED_FORMAT_VERSIONS)})")
    raw_jobs = data.get("jobs", [])
    if not isinstance(raw_jobs, list): raise JobListFormatError("Feltet jobs må være en liste")
    batch = JobBatch()
    for raw_job in raw_jobs:
        if not isinstance(raw_job, dict): raise JobListFormatError("Ugyldig jobb i jobs-listen")
    ...
    if active_job_id is not None:
        ...
        if batch.get(active_job_id) is None: active_job_id = None
    return LoadedJobList(batch=batch, active_job_id=active_job_id, created_at=str(data.get("created_at", "")),
                         modified_at=str(data.get("modified_at", "")), app_version=str(data.get("app_version", "")))
