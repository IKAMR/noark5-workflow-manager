...
class WorkflowPanel(ctk.CTkFrame):
    def __init__(
        ...
        ctk.CTkLabel(self, text="WORKFLOW", font=theme.SMALL_FONT, text_color=theme.TEXT_MUTED).grid(
        ...
        self.run_button = ctk.CTkButton(
            self, text="Kjør workflow", command=self.on_run, height=34, font=theme.NORMAL_FONT
        )
        self.run_button.grid(row=2, column=0, padx=4, pady=(10, 6), sticky="ew")

        buttons = ctk.CTkFrame(self, fg_color="transparent")
        buttons.grid(row=3, column=0, padx=4, pady=(0, 4), sticky="ew")
        buttons.grid_columnconfigure((0, 1), weight=1)
        ctk.CTkButton(buttons, text="Tøm", command=self.clear, height=28, font=theme.SMALL_FONT).grid(
            row=0, column=0, padx=(0, 3), sticky="ew"
        )
        ctk.CTkButton(buttons, text="Lagre profil...", state="disabled", height=28, font=theme.SMALL_FONT).grid(
    ...
    def refresh(self) -> None:
        ...
        for row, op_id in enumerate(ids):
            ...
            ctk.CTkLabel(item, text=f"{row + 1}. {operation.definition.name}", font=theme.SMALL_FONT, anchor="w").grid(
